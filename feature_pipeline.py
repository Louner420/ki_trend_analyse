...
class SocialTrendAnalyzer:
    ...
    def load_data_from_scraper(self, scraper_output: dict):
        """
        Konvertiert das Dictionary deines Scrapers in einen DataFrame.
        """
        ...
        # Scraped_at für alle Zeilen setzen
        if fetched_at_str:
    ...
    def feature_engineering(self, df):
        ...
        # (scraped_at muss existieren - wurde im Daemon gefixt)
        ...
        # 2. Engagement Normalisierung & Reparatur
        # --- SUPER-FIX START ---
        for col in ['likes', 'comments', 'shares', 'views']:
            # A: Wenn Spalte fehlt, erstellen
            if col not in df.columns:
                df[col] = 0
            # B: Sicherstellen, dass es Zahlen sind (kein Text!)
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
        # --- SUPER-FIX ENDE ---

        # 3. Gewichtete Summe (Total Interactions)
        # WICHTIG: Diese Zeile darf nicht fehlen!
        df['total_interactions'] = df['likes'] + df['comments'] + (df['shares'] * 2.0)
        
        # 4. Engagement Rate (Interaktionen pro View)
        # +1 im Nenner verhindert Fehler bei 0 Views
        df['engagement_rate'] = df['total_interactions'] / (df['views'] + 1)

        # 5. Viralitäts-Koeffizient (Verhältnis Shares zu Likes)
        df['viral_coefficient'] = df['shares'] / (df['likes'] + 1)

        # 6. Wachstums-Proxy (Velocity)
