from flask import Blueprint, json, render_template, request, redirect, url_for, session, current_app, jsonify
import sqlite3
import re
import time
import traceback
...
def Trend_Analyse(i):
    with open('C:\\Users\\Max\\Documents\\Schule\\5CHEL\\Diplomarbeit\\Finale Version\\website-5.3.26\\website-5.3.26\\app\\trends.json', 'r', encoding='utf-8') as f:
        trends_anal = json.load(f)
    return trends_anal
...
trend_anal=Trend_Analyse(1)

# GENERAL
general_titles, general_guides = [], []
# GASTRO
gastro_titles, gastro_guides = [], []
# FITNESS
fitness_titles, fitness_guides = [], []
# BEAUTY
beauty_titles, beauty_guides = [], []
# FASHION
fashion_titles, fashion_guides = [], []
# VLOG
vlog_titles, vlog_guides = [], []

# Daten verteilen
for kategorie, eintraege in trend_anal.items():
    for eintrag in eintraege:
        t = eintrag.get('ai_title', '')
        g = eintrag.get('ai_guide', '')
        
        if kategorie == 'general':
            general_titles.append(t); general_guides.append(g)
        elif kategorie == 'gastro':
            gastro_titles.append(t); gastro_guides.append(g)
        elif kategorie == 'fitness':
            fitness_titles.append(t); fitness_guides.append(g)
        elif kategorie == 'beauty':
            beauty_titles.append(t); beauty_guides.append(g)
        elif kategorie == 'fashion':
            fashion_titles.append(t); fashion_guides.append(g)
        elif kategorie == 'vlog':
            vlog_titles.append(t); vlog_guides.append(g)

# Beispiel-Check: Was ist in Beauty drin?
print(f"Anzahl Beauty-Titel: {len(beauty_titles)}")
print(f"Erster Beauty-Titel: {general_guides[0]}")
...
def trends():
    ...
    try:
        ...
        result, raw, themas, engaments = _empty_index_data()
        return render_template("trends.html", title="Dashboard",
            result=result, raw=raw, themas=themas, engaments=engaments, avg_velocity10=0)
    result={
        "Caption1": Cap(1),
        "Trend_Score1": trend(1),
