...
deploy_remote_file() {
	...
	if [[ -f "$source_path" ]]; then
		ssh "${SSH_OPTS[@]}" "$SERVER_IP" "mkdir -p '$REMOTE_DIR/$(dirname "$target_rel_path")'"
		scp "${SSH_OPTS[@]}" "$source_path" "$SERVER_IP:$REMOTE_DIR/$target_rel_path"
	else
...
deploy_dir_py() {
	...
	ssh "${SSH_OPTS[@]}" "$SERVER_IP" "mkdir -p '$REMOTE_DIR/$target_dir_rel'"
	while IFS= read -r -d '' file; do
		local rel_path="${file#$source_dir/}"
		local target_rel_path="$target_dir_rel/$rel_path"
		ssh "${SSH_OPTS[@]}" "$SERVER_IP" "mkdir -p '$REMOTE_DIR/$(dirname "$target_rel_path")'"
		scp "${SSH_OPTS[@]}" "$file" "$SERVER_IP:$REMOTE_DIR/$target_rel_path"
...
verify_remote_file() {
	...
	if ! ssh "${SSH_OPTS[@]}" "$SERVER_IP" "test -f '$REMOTE_DIR/$target_rel_path'"; then
		echo "❌ Verify fehlgeschlagen, remote fehlt: $target_rel_path"
		return 1
	fi
	local_size="$(wc -c < "$source_path" | tr -d ' ')"
	remote_size="$(ssh "${SSH_OPTS[@]}" "$SERVER_IP" "wc -c < '$REMOTE_DIR/$target_rel_path'" | tr -d ' ')"
...
verify_deploy_batch() {
	...
	verify_remote_file "$HOME/projects/ai/content_agent.py" "content_agent.py" || failed=1
	...
	verify_remote_file "$HOME/projects/ai/advanced_ai/archive/check_db.py" "archive/check_db.py" || failed=1
	if [[ "$failed" -ne 0 ]]; then
...
REMOTE_HOME="$(ssh "${SSH_OPTS[@]}" "$SERVER_IP" 'printf %s "$HOME"')"
if [[ -z "$REMOTE_HOME" ]]; then
	echo "❌ Konnte Remote-HOME nicht ermitteln."
	exit 1
fi
REMOTE_DIR="$REMOTE_HOME/$SERVER_DIR_REL"
ssh "${SSH_OPTS[@]}" "$SERVER_IP" "mkdir -p '$REMOTE_DIR/data'"
...
if [[ "$DEPLOY_CODE" == "1" ]]; then
	...
	deploy_remote_file "$HOME/projects/ai/content_agent.py" "content_agent.py"
	...
	deploy_remote_file "$HOME/projects/ai/advanced_ai/archive/check_db.py" "archive/check_db.py"
...
scp "${SSH_OPTS[@]}" "$HOME/projects/database/raw_tiktok.db" "$SERVER_IP:$REMOTE_DIR/data/raw_tiktok.db"
scp "${SSH_OPTS[@]}" "$HOME/projects/database/users.db" "$SERVER_IP:$REMOTE_DIR/data/users.db"  # NEU!

echo "🧠 [2/3] Wecke die Heavy-AI (Clustering) auf dem Server..."
# SSH-Aufruf 1: Nur das Clustering. Mit -u sehen wir es live!
ssh "${SSH_OPTS[@]}" "$SERVER_IP" "cd '$REMOTE_DIR' && source venv/bin/activate && AI_RUN_ONCE=1 python3 -u daemon_ai.py"

echo "🤖 Starte LLM Content-Agent (Llama 3)..."
# SSH-Aufruf 2: Nur die Textgenerierung. Verhindert SSH-Timeouts.
ssh "${SSH_OPTS[@]}" "$SERVER_IP" "cd '$REMOTE_DIR' && source venv/bin/activate && python3 -u content_agent.py"

echo "📥 [3/3] Lade veredelte Datenbank UND alle User-JSON-Dateien zurück..."
scp "${SSH_OPTS[@]}" "$SERVER_IP:$REMOTE_DIR/data/raw_tiktok.db" "$HOME/projects/database/raw_tiktok.db"
if ssh "${SSH_OPTS[@]}" "$SERVER_IP" "test -f '$REMOTE_DIR/data/trend_results.db'"; then
	scp "${SSH_OPTS[@]}" "$SERVER_IP:$REMOTE_DIR/data/trend_results.db" "$HOME/projects/database/trend_results.db"
elif ssh "${SSH_OPTS[@]}" "$SERVER_IP" "test -f '$REMOTE_HOME/database/trend_results.db'"; then
	scp "${SSH_OPTS[@]}" "$SERVER_IP:$REMOTE_HOME/database/trend_results.db" "$HOME/projects/database/trend_results.db"
else
...
scp "${SSH_OPTS[@]}" "$SERVER_IP:$REMOTE_DIR/data/trends_user_*.json" "$HOME/projects/database/"  # NEU: Lädt alle User-Dateien!
