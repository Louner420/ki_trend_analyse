...
cd ~/projects/ai && source venv/bin/activate && python3 data/clean_db.py

echo "🚀 [1/3] Sende Datenbanken zum Schulserver..."
scp ~/projects/database/raw_tiktok.db $SERVER_USER@$SERVER_IP:$SERVER_DIR/data/raw_tiktok.db
scp ~/projects/database/users.db $SERVER_USER@$SERVER_IP:$SERVER_DIR/data/users.db  # NEU!
...
scp $SERVER_USER@$SERVER_IP:$SERVER_DIR/data/raw_tiktok.db ~/projects/database/raw_tiktok.db
scp $SERVER_USER@$SERVER_IP:$SERVER_DIR/data/trends_user_*.json ~/projects/database/  # NEU: Lädt alle User-Dateien!
